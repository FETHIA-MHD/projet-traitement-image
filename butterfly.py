...
def ConversionNB (Img):
    """Convertie l'image Img en noir et blanc"""
    n =len (Img)#nombre de lignes dans Img
    p =len (Img [0])#nombre de colonnes dans Img
    for i in range(n):
        for j in range (p):
...
img=imageio.imread("butterfly.jpg").tolist()
print(len(img))
print(img[4][122])
print(len(img[4]))
...
# ConversionNB(img)
# plt.figure()
# plt.imshow (img,cmap="gray")pip
# plt.title("Image Noire et blanc")
...
def DessineContours (Img):
    #"""Retourne une image qui va représenter les contours de Img":

    n =len (Img)#nombre de lignes dans Img
    p =len (Img [0])#nombre de colonnes dans Img
    P=[[0 for j in range (p)] for i in range(n)]#Z la matrice nulle
...
image = cv2.imread('butterfly1.jpg', cv2.IMREAD_GRAYSCALE)
...
#p=np.polyfit(x ,y , 3)
#print(p)
#l= np.linspace(0,120,200)


#plt.plot (x, -2.04407316*x**3 - -9.28484430*x**2 +1.5069759300*x-2.3518378902, c = ' b', label = ' P(x)')
#plt.legend ()
#plt.show ( )
#plt.scatter(h_x1,)
...
#plt.scatter(h_xin0,h_yin0, label='bo1',c='BLACK')
#plt.legend()
#plt.plot(h_xin0,fil,label='p1(x)',c='orange',marker='*')
#plt.legend()
"""plt.scatter(h_xin1,h_yin1, label='bo2',c='red')
 
plt.plot(h_xin1,fil1,label='p2(x)',c='BLUE' ,marker='+')
plt.legend()
plt.scatter(h_xin2,h_yin2, label='bo2',c='YELLOW')
plt.plot(h_xin2,fil2,label='p3(x)',c='GREEN',marker='*')
plt.legend()"""
